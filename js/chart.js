    ...
    function createGradient(ctx, color, isUp) {
      ...
      if (isUp) {
        gradient.addColorStop(0, color === 'green' ? 'rgba(16, 185, 129, 0.45)' : 
                                    color === 'red' ? 'rgba(239, 68, 68, 0.45)' : 
                                    'rgba(156, 163, 175, 0.45)');
        gradient.addColorStop(1, color === 'green' ? 'rgba(16, 185, 129, 0)' : 
  ...
  if (Math.abs(difference) < epsilon) {
    ...
    function generateMonthData(testDate) {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        fullLabels.push(`${monthNames[currentDate.getMonth()]} ${currentDate.getDate()} (${dayNames[currentDate.getDay()].substring(0, 3)})`);
    ...
    //       if (!cachedMonthData || 
    //           cachedMonthData.endDate.getTime() !== endDate.getTime()) {
    ...
    //       valueText.innerHTML = `${weekTrendIndicator}`; // Show trend indicator in value text 
    //       break;
    ...
    //       infoText.innerHTML = `${formatDate(result.startDate)} - ${formatDate(result.endDate)}`;
    //       valueText.innerHTML = `${monthTrendIndicator}`; // Show trend indicator in value text 
    ...
    //       valueText.innerHTML = `${yearTrendIndicator}`; // Show trend indicator in value text 
    //       break;
    //   }
    ...
    const chart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          borderColor: initialTrendColor === 'green' ? '#10b981' : 
                       initialTrendColor === 'red' ? '#ef4444' : 
                       '#9ca3af',
          borderWidth: 2,
          tension: 0.4,
          pointRadius: 0,
          pointHoverRadius: 4,
          pointHoverBackgroundColor: initialTrendColor === 'green' ? '#10b981' : 
                                     initialTrendColor === 'red' ? '#ef4444' : 
                                     '#9ca3af',
          spanGaps: true
        }]
...
if (weekButton) {
  weekButton.classList.add('active');
    ...
    // Update chart when test date changes
    ...
    function updateChartWithData(newData) {
      ...
      chart.data.datasets[0].borderColor = trendColor === 'green' ? '#10b981' : 
                                           trendColor === 'red' ? '#ef4444' : 
                                           '#9ca3af';
      chart.data.datasets[0].pointHoverBackgroundColor = trendColor === 'green' ? '#10b981' : 
                                                         trendColor === 'red' ? '#ef4444' : 
                                                         '#9ca3af';
    ...
    function testDataConsistency() {
      console.log("Testing data consistency between 7-day and 30-day views:");
      console.log("==========================================================");
      
      const testDate = new Date('2024-12-31');
      testDateInput.value = testDate.toISOString().split('T')[0];
      
      // Generate month data
      const monthData = generateRandomData('month', testDate);
      console.log(`Month data (30 points):`);
      console.log(`  Last 7 values: ${monthData.data.slice(-7).map(v => v.toFixed(2))}`);
      
      // Generate week data
      const weekData = generateRandomData('week', testDate);
      console.log(`\nWeek data (7 points):`);
      console.log(`  All values: ${weekData.data.map(v => v.toFixed(2))}`);
      
      // Compare the data
      const monthWeekData = monthData.data.slice(-7);
      const weekWeekData = weekData.data;
      
      let match = true;
      for (let i = 0; i < 7; i++) {
        if (Math.abs(monthWeekData[i] - weekWeekData[i]) > 0.01) {
          match = false;
          console.log(`  Mismatch at position ${i}: month=${monthWeekData[i].toFixed(2)}, week=${weekWeekData[i].toFixed(2)}`);
        }
      }
      
      if (match) {
        console.log(`\n✓ SUCCESS: 7-day data matches last 7 points of 30-day data`);
      } else {
